...
# Este bloque de código se encarga de transformar todas las fechas de strings, a dates.
...
# Este bloque de código se encarga de eliminar de la lista de lesionados a los jugadores que
# estén disponibles a día de hoy
for lesionado in lesionados: 
    if lesionado['regreso_estimado'] < date.today():
        print(f"AVISO: {lesionado['nombre']}({lesionado['dorsal']}) ya no está lesionado.")
        disponible = {'dorsal' : lesionado['dorsal'], 'nombre': lesionado['nombre'], 'lesion' : "N/A"}
                    ...
                    if (nuevo_lesionado['semanas_de_baja'] <= 0):
                        ...
                        while (respuesta != "si") or (respuesta != "no"): #Esto no está completo. Estoy intentando perfeccionarlo
                            ...
                            if (respuesta.strip() == "si") or (respuesta.strip == "sí"):
                                ...
                                print("Las semanas de baja deben ser mayor o igual a 1.")
                                las_semanas_de_baja_tienen_sentido = False
                                break
                            else:
...
for lesionado in lesionados:
    if type(lesionado['regreso_estimado']) == date:
        lesionado['regreso_estimado'] = lesionado['regreso_estimado'].strftime("%d-%m-%Y")
